...
def send_active_email(request, email):
    ...
    active_path = (
        reverse("staff:active_staff") + "?" + parse.urlencode({"token": token})
    )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            user = OAUser.objects.create_user(
                email=email, realname=realname, password=password
            )
            ...
            return Response(
                data={"detail": list(serializer.errors.values())[0][0]},
...
class StaffDownloadView(APIView):
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": "员工参数错误"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            if current_user.department.name != "董事会":
                if current_user.department.leader_id != current_user.uid:
                    return Response(
                        {"detail": "没有权限下载"}, status=status.HTTP_403_FORBIDDEN
                    )
                else:
            ...
            result = queryset.values(
                "realname", "email", "department__name", "date_joined", "status"
            )
...
class StaffUploadView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            current_user = request.user
            if (
                current_user.department.name != "董事会"
                or current_user.department.leader_id != current_user.uid
            ):
                return Response(
                    {"detail": "您没有权限访问"}, status=status.HTTP_403_FORBIDDEN
                )
            ...
            for index, row in staff_df.iterrows():
                ...
                if current_user.department.name != "董事会":
                    ...
                    try:
                        department = OADepartment.objects.filter(
                            name=row["部门"]
                        ).first()
                        if not department:
                            return Response(
            ...
            try:
                ...
                return Response(
                    {"detail": "员工数据添加错误"}, status=status.HTTP_400_BAD_REQUEST
                )
